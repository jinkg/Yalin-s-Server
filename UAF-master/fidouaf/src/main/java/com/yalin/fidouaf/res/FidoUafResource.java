...
import com.yalin.fidouaf.RPserver.msg.GetUAFRequest;
import com.yalin.fidouaf.RPserver.msg.ReturnUAFAuthenticationRequest;
import com.yalin.fidouaf.RPserver.msg.ReturnUAFDeregistrationRequest;
import com.yalin.fidouaf.RPserver.msg.ReturnUAFRegistrationRequest;
import com.yalin.fidouaf.RPserver.msg.ServerResponse;
import com.yalin.fidouaf.facets.Facets;
import com.yalin.fidouaf.facets.TrustedFacets;
import com.yalin.fidouaf.res.util.DeregRequestProcessor;
import com.yalin.fidouaf.res.util.FetchRequest;
import com.yalin.fidouaf.res.util.ProcessResponse;
import com.yalin.fidouaf.res.util.StorageImpl;
import com.yalin.fidouaf.response.ResponseBuilder;
...
public class FidoUafResource {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public String getRegReqForAppId(@PathParam("username") String username, @PathParam("appId") String appId) {
		RegistrationRequest[] regReq = getRegisReqPublic(username);
		setAppId(appId, regReq[0].header);
		return ResponseBuilder.buildStartRegistrationResponse(regReq);
	}
	...
	private String[] getAllowedAaids() {
		String[] ret = { "EBA0#0001", "4e4e#4005", "0015#0001", "0012#0002", "0010#0001", "4e4e#0001", "5143#0001",
				"0011#0701", "0013#0001", "0014#0000", "0014#0001", "53EC#C002", "DAB8#8001", "DAB8#0011", "DAB8#8011",
				"5143#0111", "5143#0120", "4746#F816", "53EC#3801" };
		return ret;
	}
	...
	public Facets facets() {
		...
		String[] trustedIds = { "https://www.head2toes.org", "android:apk-key-hash:Df+2X53Z0UscvUu6obxC3rIfFyk",
				"android:apk-key-hash:bE0f1WtRJrZv/C0y9CM73bAUqiI", "https://openidconnect.ebay.com" };
	...
	@Produces(MediaType.APPLICATION_JSON)
	public String processRegResponse(String payload) {
		...
		return ResponseBuilder.buildFinishRegistrationResultData(result);
	}
	...
	public String getAuthForAppIdReq(@PathParam("appId") String appId) {
		...
		return ResponseBuilder.buildStartAuthenticateData(authReqObj);
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public String processAuthResponse(String payload) {
		...
		return ResponseBuilder.buildFinishAuthenticateResultData(result);
	}
	...
	public ServerResponse UAFAuthResponse(String payload) {
		...
		findOp = findOp.substring(findOp.indexOf("op") + 6, findOp.indexOf(",", findOp.indexOf("op")) - 1);
